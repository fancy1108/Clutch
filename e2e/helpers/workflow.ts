...
export async function ensureE2eUserWorkflow(): Promise<string> {
  ...
  const res = await fetch(`${sidecarBaseUrl()}/api/workflows/user`, {
    method: 'POST',
...
export async function startVideoProductionRun(instruction: string): Promise<string> {
  const authorize = await fetch(`${sidecarBaseUrl()}/api/workspace`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  const start = await fetch(`${sidecarBaseUrl()}/api/runs/start`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
