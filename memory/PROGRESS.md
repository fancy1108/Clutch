...
- **Handoff 质量提升与安全检测保护（智能体原生 Handoff 触发）**：为了兼顾未安装 `/handoff` 技能的用户，我们在后台增加了安全检测保护。当触发交接时，后台会首先通过 `is_handoff_skill_installed` 检查用户的全局和项目技能目录下是否安装有 `/handoff` 技能。只有在检测到该技能存在时，才会自动向源 Agent 终端 PTY 中注入 `\n/handoff\n` 命令，并在 3.0s 后检索 OS 临时目录中的交接文件并读取为 `SOURCE OUTPUT`；如果用户未安装该技能，后台将**直接跳过命令注入和等待**，无缝使用外部 LLM 多轮聊天与 PTY 日志进行 Fallback 摘要。这彻底避免了未安装该技能的用户在终端产生未知指令报错或 prompt 污染。同时 LLM 超时也设为 `10.0`s 保障总结质量。
