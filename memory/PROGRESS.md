...
- **Handoff 质量提升（智能体原生 Handoff 触发）**：整合了智能体原生交接能力。当触发交接时，后台会自动先向源 Agent 终端 PTY 中注入 `\n/handoff\n` 命令，触发其调用自身的 Handoff 技能总结终端内的全部聊天上下文（包含未通过 Bar 发送的命令和输出）并写盘至 OS 临时目录；后台等待 3.0s 后检索并读取该临时文件作为正文的 `SOURCE OUTPUT`，彻底避免了外部大模型因不知道终端内交互内容而生成空摘要的问题，并保留原外部 LLM 多轮聊天摘要作为高层规划的 Fallback。同时配置 LLM 超时为 `10.0` 秒给大模型更充裕时间。
