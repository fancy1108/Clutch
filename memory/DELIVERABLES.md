...
- **证据：** —
- **交付文件：**
  - `services/orchestrator/src/handoff_summarizer.py` — Added intelligent TUI visual noise cleaning, integrated current chat history context, added OS temp directory scanning, and implemented `is_handoff_skill_installed` detection.
  - `services/orchestrator/src/llm/router.py` — Propagated `timeout_sec` and `max_tokens` keywords down to `http_chat_complete`.
  - `services/orchestrator/src/main.py` — Decoupled summarization into an async background task, fixed target log entry index mapping bugs, enabled system-wide active PTY CLI scan, and safely injected `\n/handoff\n` to let the source agent generate its own handoff summary in PTY only if the handoff skill is installed.
