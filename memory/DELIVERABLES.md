...
- **交付文件：**
  ...
  - `services/orchestrator/src/main.py` — Decoupled summarization into an async background task, fixed target log entry index mapping bugs, enabled system-wide active PTY CLI scan, and injected custom system instructions for CLI agents to generate summaries dynamically while polling for files.
