...
* **Workspace Chrome**：左侧项目/会话侧栏支持 260px 展开与 56px 纯图标折叠，折叠按钮位于左侧面板右边缘中线位置，与右侧监督面板折叠按钮保持一致；折叠态项目和导航通过 tooltip 展示全名。Chat 主区根据侧栏/右侧监督面板开合动态收窄或扩展，右侧监督面板 Tab 采用等分网格与底部短指示条，保持 Windows 桌面窗口下的信息密度与稳定布局。
