...
* **Rich Chat Input Bar & Attachments**：支持从剪贴板直接粘贴图片生成 Chip 缩略图预览；支持从右侧文件树拖拽文件/文件夹进入输入框作为附件；输入框内键入 `/` 触发已扫描 Skills 的指令联想，键入 `#` 触发历史会话引用联想；工作流精修模式（`refining` 或已完成工作流）下键入 `@` 触发工作流 Agent 联想；提供全局运行状态控制（Running 时展示 Stop 按钮，工作流 Stop 进入精修而非 failed）；提供持久化的安全审批模式选择（Auto-approve, Ask-on-Write, Manual confirmation）。**Coding 发图**：Chat 底栏与 Terminal **OrchestratorBar**（非 xterm 黑框）均可粘贴/选择图片；**任意聊天 LLM 先直接发 multimodal 图**，有效回复直接展示；仅当模型软拒绝视觉或 API 拒图时，才降级本地 OCR/调色板分析。Terminal 发图时先落盘 `.clutch/attachments/`（目录含 `.gitignore` 为 `*`，过大时清理 3 天前旧图），再将路径与描述注入目标 Lane；发送期间输入与发送按钮 Loading，防重复派发。
* **中间产物点击预览**：Chat 气泡中的工作区路径、`[file:]` / `@path`、fenced 代码块可点击打开全屏预览（大文件 plain view 降级）；Terminal xterm 与派发历史中的路径/带扩展名文件名同样可点，经 sidecar resolve（精确路径 → basename 唯一模糊）校验存在后打开，不追踪 xterm CWD。
