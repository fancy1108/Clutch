...
* **界面代码层（UI code，D39 唯一 SSOT）**：入口在 **Preview Demo → Coding**（不自动弹出；画布底栏不再放「界面代码」按钮）。面板按 **4 步引导**：批准原型 → 生成界面代码（生成中显示加载动画与提示）→ 预览并批准（小窗按设计稿宽度缩放适配，可「完整尺寸打开」）→ 交给编码。产物为会话目录 `react/`（Vite + React + Tailwind）。若 PreviewDemo 已落盘 `interaction_contract.json`，生成时并入导航约束。同路径写入 `react/`，不以 `generated/` 交接。Windows 下 preview 会解析 `pnpm.cmd` / `npm.cmd` / `npx.cmd` 完整路径，停止或重新生成时清理 Vite/Node 进程树，避免端口残留。
