...
> **This document describes WHAT users can do** in **v1.4.1**.  
> GitHub Release: [v1.4.1](https://github.com/fancy1108/Clutch/releases/tag/v1.4.1).  
...
- **Clutch 架构解法（现状与路线图）**：
  - **当前已实现**：支持基础的 **State 跨会话持久化与恢复**（`states/{run_id}.json`）、针对 CLI 引擎的 **Session 恢复机制**（`--resume` / Codex history replay），以及 **长会话消息压缩（Compaction，capability D8）**：当估算 token 接近上下文窗口限额时自动折叠中间轮次，完整原文归档至 `runs/archive/{run_id}.jsonl`，Chat 展示压缩摘要；**Todo / 计划要点**钉进摘要并每轮以末尾 `<agent_status>`（含本机时钟）再注入，压缩后仍可回答「还剩什么」。
...
* **Subtask delegation（D10 + D48）**：Clutch Agent 可调用 `delegate_subtask` 派发 **explore**（只读）或 **implement** 子任务；父气泡下嵌套 **Subtasks** 卡展示状态、摘要与可展开步骤；子失败在父卡可见。Explore 子循环默认更高工具步数预算；用户要求「先探再改」时至少出现 explore 卡，implement 可由父 Agent 完成（不强制第二张卡）。子任务交差回主 Agent；要问人时主 Agent 走 Question 卡（`ask_user_question`），没有单独的 Notify 卡。已有子任务在跑时再派一个，直接开，不再弹并行确认。
...
* **Foreground → background（D34）**：长命令前台执行时，输入栏上方出现 **Move to background**；一键转入 D11 后台列表后可继续聊天（对标 Grok Ctrl+B）。前台超过 `timeout_sec`（默认 60s）会杀掉进程组并出解释器超时卡，而不是只杀 shell、留下孤儿子进程。
...
* **Interpreter errors（FM-17 / B-47）**：`run_terminal_cmd` 超时或 shell 无法启动时，Chat 输入框上方出现结构化卡（超时 / 离线，不是 traceback）。没有 **+** 菜单预览。
...
* **Cross-session memory（D16 / B-39 / B-40 / B-45 / FM-12）**：Settings 可开/关 **Memory**。你说「记住：…」后，工作区 **Files → `.clutch/memory/MEMORY.md`** 会出现可编辑笔记（下一会话会读这份概览）。交差验证卡通过/失败时也会记一行 `Worked:` / `Failed:`。网页里「请记住 + 链接」或只丢一个 URL **不会**写入 MEMORY.md，也不会写入 Settings Memory（工具会拒绝，Agent 不能说「已保存」）。可 **Clear memory** 清全局 JSON，项目文件需自己删。**General → Search** 可检索 `.clutch/memory`，点结果会打开该文件预览。
...
* **Chat / Terminal 双模式（Single Agent plain chat）**：当 Active Agent 为 **任意已 Connect 的 CLI 类型**（`*-cli` 路由键，如 Claude Code、OpenCode、Codex、Aider、CodeBuddy、Rivet、Ollama、Antigravity 等；不含 Clutch 内置 LLM）时，主工作台右上角可切换 **对话模式**（默认）与 **终端模式**（Terminal Orchestra：多 Lane 嵌入式 xterm + `INTERACTIVE_PTY`；**Windows 通过 WinPTY 后端附着交互式 CLI**）。**终端模式下底部 Agent 下拉仅列出 CLI 类型 Agent**。终端模式下底部 `ChatInputBar` 隐藏，改为 **OrchestratorBar**（与 Chat 输入栏同款 `+` / ✋ / `@` Agent 选择，**`fixed bottom-8` 贴底布局**）；支持 `@目标Agent` 自然语言或图语法派发、**发送即派发**（跨 Agent 交接用 `@C from @A` 写在同一条里；无确认卡 / 排队条 / Complete 草稿，D67）；**同一 CLI 再次 `@` 注入已有 Lane**；handoff 文件写入与 Lane 间可视化（Orchestrator `@` 提及与已路由 CLI 显示名一致）。**OrchestratorBar 发送后，系统将任务文本注入目标 Lane 的 PTY（等待 TUI 就绪后自动提交 Enter）**；Ollama 以 `ollama run <model>` 启动交互会话。展开 Lane 超过 4 个时，终端区顶部显示分页圆点（每页 2×2）；**收起、翻页或侧栏切换时 Lane PTY 保持连接，xterm 从缓冲 transcript 恢复并重绘，避免黑屏**。派发确认后 Overview **即时以 Loading 步骤形式（「正在生成交接摘要…」、「正在打开终端…」、「正在注入任务目标…」）显示派发记录并支持异步更新**，时间戳按用户本地时区显示。Overview **保存为工作流**（FM-07）按派发顺序生成 SOP，Workflows 列表可见。Handoff「Send to Bar」填入图语法 `@目标 from @来源 @handoff.md`（**对话模式**输入栏同样接收该事件并显示草稿芯片，FM-08）；切到终端时若输入为空会带上近期对话。；Handoff 预览不再出现标题下空白卡片。用户可在 xterm Lane 内直接输入，或通过 OrchestratorBar 派发到其它 Lane；切回对话模式后历史消息与监督发消息能力不变，PTY 会话在后台保持以减少切换卡顿。工作流 session 不展示切换。右侧监督面板 Tab 为 **Overview · Files · Changes · Terminal**（Overview 文本可选中复制）；Overview 展示派发记录（**含 CLI Session 恢复命令与 Copy**：Claude/CodeBuddy `claude --resume`、Codex `codex resume --last`、OpenCode `opencode -c` / `opencode -s`、MiMo Code `mimo -c` / `mimo -s`、ZCode `zcode --resume sess_...` / `zcode -c`、Ollama/Antigravity 等按 Agent 类型生成，Clutch 内部分配的 UUID 不误用于不支持 resume 的 CLI）；Terminal 审计日志独立 Tab 保留。
...
|------------|----------------------|
| D1 builtins | Verb-group tool trail (D46) + files-changed chips (D47) + Allow/Reject for risky writes/shell. Filename existence (README.md) → **List**, not Search (B-41) |
| D2–D4 plan/todo/ask | In-chat Plan / Todo / Question cards (D49); Todo sticky + chevron collapse; Plan per-step comments (D31) |
| D5–D6 verify/diff | Verification report card + Cursor-style Diff cards (D50). Claiming passed runs the workspace test suite and checks listed files exist (B-37). Remember/Q&A turns do not publish a leftover card |
...
| D34 fg → bg | Foreground bar + **Move to background**; timeout kills the process group and shows the interpreter card; same command refused if already running |
...
| D15 web search | Settings **Allow network** (default **On**) → builtin `web_search` (DuckDuckGo HTML); off = tool hidden |
...
| D39 transport | Hub register: stdio command **or HTTP URL**; Env `KEY=value` / `Authorization`; HTTPS 401 without a key tries `mcp-remote` OAuth. Figma remote MCP allowlists clients (Claude Code / Cursor / …) — Clutch gets 403, not a login page |
...
| D46 tool trail | Unified process rows (title + focus); todos only on Todo card; expand = capped result (no scroll jump) |
| D47 chips | Clickable files-changed chips → workspace preview; generated `.html` opens in **system browser** |
...
| D68 reply style | Clutch Agent `style` layer: lead with action/answer; no preamble/closers; errors = cause + fix. Full ADHD skill optional via `read_skill` |
...
* **General Settings**：支持用户修改个人名称并应用在发送气泡标签中；支持上传自定义头像并转换为 base64 存盘；支持小/默认/大/特大/超级大字体大小偏好并持久化（`data-font-size`）；支持中英文双语对照切换，后端 API / WS 错误采用 `tr()` 响应；**默认工作区**可在 General 选择，启动时自动激活（未选则沿用上次）；**停止运行前确认**开关持久化（开：对话 Stop 每会话询问一次；关：立即停止；工作流 Stop 从不确认）；**未信任 MCP 确认**开关（开：启用 Hub MCP 时询问一次并记入本机信任列表；关：不再询问。自己创建的工作流点选即绑定，不询问）；General 与底栏均用 Tauri `getVersion` 显示桌面版本号（底栏版本贴右，不随字段挤出）。
* **Agent Settings**：提供可视化 Agent 管理器（`AgentManager.tsx`），支持自由增删改自定义 Agent，配置其名称、头像、**可编辑协议段**（`markdownDoc`，capability **D53** 中仅作 protocol 层，不再假装等于完整 runtime system）、模型及关联 MCP 工具。运行时 system 由分层组装：底座 / Env（系统、Shell、工作区；**不含**时钟）/ **回复风格**（**D68**：第一句给下一步或答案；禁止开场白与收尾客套；报错只写原因和修法；不估计分钟数、不复述 `<agent_status>`、未点名不要配图。完整 ADHD 输出风格仍走可选 Skill / `read_skill`，不进默认提示词）/ 协议 / **项目规则**（**D7**，对齐 Grok：从 git 根走到授权工作区路径，加载 `AGENTS.md`·`CLAUDE.md` 与 `.grok/.claude/.cursor/rules`，更深优先；无 User home 规则）/ **Skills 开放目录**（全局∪当前仓∪自定义 SEARCH PATHS 中 Enabled 项自动进 catalog，同名项目优先；Agent 绑定可选叠加；全文经 `read_skill`）/ Plan 模式当轮 reminder；本机 **Local time** 与完整 Todo/计划在对话末尾 `<agent_status>`（每轮整换，不进 system 前缀，B-35）。切换工作区时仅轮换项目自动 Skills 挂载，手动 Search Paths 保留。**Agent Manager → Clutch Agent 详情右侧**可查看「运行时提示词分层」（层名+字符量，可刷新）；亦可用 `GET /api/agents/{id}/prompt-assembly`。**Skills / MCP 模块按 Agent 类型分档**：仅 **Clutch** 内置 Agent 可绑定 Clutch Skills Registry 与 MCP Hub（Module 4 可勾选 Hub 服务器并持久化 `mcpServerIds`）；**Claude Code** / **OpenCode** / **MiMo Code** CLI Agent 展示各自原生配置只读扫描与 Settings 深链；其他 CLI 类型显示「即将上线」，避免误用全局 Registry。
* **Clutch Agent 内置手脚（capability D1 / DECISIONS D44）**：授权工作区后，Chat 选择 **Clutch Agent** 默认挂载虚拟 MCP **`clutch-tools`**（无需先绑 Hub）：`read_file` / `list_dir` / `grep` / `search_replace` / `run_terminal_cmd` / `apply_patch` / **`propose_plan`** / **`todo_write`** / **`ask_user_question`** / **`submit_verification`** / **`submit_diff_summary`**。写文件与 shell 仍走既有风险审批（`permission_mode`）。额外 Hub 服务器（如 `local-fs`）可在 Agent Manager Module 4 绑定叠加。Chat 输入框 **+** 菜单显示 **已绑 N 个 MCP / ~M 工具**（capability **D40**）；未绑时「绑定 MCP」引导至 Agent Manager。运行中与结束后，Chat 以统一的 **过程步骤行**展示工具轨迹（capability **D46**：状态图标 + 可读标题 + 一行 focus（URL/路径/查询）；点开仅展开限高结果，避免滚动跳动；`todo_write` 不进步骤流，只看顶部 Todo 卡；运行中底部 **Working…**）。按文件名查是否存在（如 README.md）走 `list_dir`，步骤条显示 **List**，不会对文件名 `grep` 搜内容（B-41）。本回合写出的工作区文件会以 **变更文件芯片**（capability **D47**）出现在气泡下方；点击走全屏预览（**DECISIONS D42**）。Chat 会从自然语言需求**拆解意图并推断交付形态**（网页 / 图 / 视频 / 代码 / 回答），不必用户点名文件类型（含「信息图 / infographic / 可视化」→ 图）。要图/视频时，回合结束会**自动调用设置里已配置的图像/视频模型**（如 Agnes Image / Agnes Video），产出落在 **`.clutch/generated/images/`** 或 **`.clutch/generated/videos/`**，无需在 footer 手动切换模型；若未配置 Key，对话会明确提示**最后一步失败**及如何补 Key。禁止用 HTML 冒充图/视频。研究/展示类新文件默认进 **`.clutch/artifacts/`**。仅当推断为可浏览页面且本回合新写出页面时，才可能自动用系统浏览器打开。工具步骤与子任务卡提供 **在 Terminal 查看**（capability **D51**）。
* **忽略规则与严格沙箱（capability D21）**：内置 `read_file` / `list_dir` / `grep` 尊重工作区 `.gitignore` 与 `.clutchignore`（忽略路径不出现在列表/搜索结果中，直接读取返回可读错误）。**Settings → General → Strict sandbox（严格沙箱）** 开启后，越界路径与试图逃出工作区的 shell 命令会被拒绝并说明原因。**Settings → General → Allow network（允许联网）** 默认开启；Clutch Agent 可使用内置 `web_search` 检索网页（D15）。关闭后隐藏 `web_search`，仍可用 `web_fetch`。对天气/实时、读文件、改代码、git、跑命令等意图，若模型空答不调工具，ReAct 会按工具族 nudge 并 `tool_choice=required` 重试一次。
* **先计划再动手（capability D2 + D49）**：多步/功能类任务时，Agent 先调用 `propose_plan`；对话流内出现 **计划卡**（步骤列表）与 **批准计划 / 修改计划 / 取消**。未批准前不执行写文件或可变 shell；简单问答可跳过计划。用户在对话中说 **确认 / 批准 / 按计划 / 按照你说的** 后，系统会提醒并强制走工具：先 `todo_write` 再 `apply_patch`/`search_replace` 执行——**不再反复要确认**，也不允许只复述计划就宣称完成。
* **Todo 清单（capability D3 + D49）**：多步执行时 Agent 调用 `todo_write`（**不走人工审批**，仅更新会话 Todo UI）；Chat 时间线展示待办/进行中/完成状态。**回合进行中**（含已全部打钩）Todo 卡吸顶固定在对话区顶部，宽度铺满聊天内容列并带不透明遮罩，避免右侧历史气泡从旁漏出；**回合结束后**取消固定，seal 到气泡 `todoList` 并随历史滚动。吸顶与时间线 Todo 卡均支持 **chevron 折叠/展开**（默认展开；折叠后仅保留标题与 `done/total`；样式走 Plan/Question 同源 `chatAgentCard` token）。计划卡步骤会去掉模型自带的 `1.` 前缀，避免显示成 `1. 1. …`。
...
* **自检报告（capability D5 + D50 / B-37）**：实现收尾时 Agent 调用 `submit_verification`；对话内出现 **验证报告卡**（步骤通过/失败 + 总结）。结论为失败时列出可操作的下一步；Todo 未完成时禁止谎称通过。工作区有测套时 harness 会先跑测试，失败则卡上多一步失败、不能绿；列出的改动文件不在磁盘上同样不能绿（不另开一张卡）。**记住 / 问答 / 未改文件**的回合不发这张卡，也不会把上一轮的卡贴到新回复上（MEMORY.md 不因此多写 `Worked:`）。卡上 **查看改动** 可打开本回合变更文件预览（与 D47 芯片同源）。
* **校验失败条（FM-18 / B-05）**：工作流节点校验失败时 Chat 出现 **VALIDATION FAILED** 文案，Overview 同步可读条（含失败原因）；图路由仍读该结果。
* **Diff 审查（capability D6 + D50）**：每次成功改文件后，对话流**立即**出现单文件 Diff 卡（文件名 + 红绿 hunk，对标 Cursor 边改边看）；可选 `submit_diff_summary` 出多文件汇总卡。与右侧 Changes 面板并存。**即使工作区没有 git**，本回合经 `apply_patch` / `search_replace` /（含 `cat >` 等）shell 写入的文件仍会计入 Changes 与气泡变更芯片；写文件应优先用 patch 工具（shell 改文件通常没有 hunk Diff 卡）。
* **Workflow Settings**：管理和选择可用的流程图 SOP 模板；Settings → Workflows SOP 点选一行即打开画布并绑到对话（名称完整换行；悬停仅编辑/删除）。底栏工作流菜单同样可选 SOP。内置模板含 `weather-to-vision`、`video-production`、**Design to Code**（`design-to-code`，Design 批准后交给 Builder）与社区贡献的 **Memory-Augmented Pipeline (Epicode)**（`epicode-memory-pipeline.json`；需自行配置 Epicode MCP，见 [`docs/mcp-servers/epicode.md`](./mcp-servers/epicode.md)）。含 `check` / `human_gate` / 条件边的复杂流程会强制 **JSON 编辑模式**；提示条会点名导致降级的节点 id / 边 id（如 `review-gate (human_gate)`、`edge e5 when:reject`），不再只显示泛化的「复杂流程」。`check(file_exists)` 的 `path` 必须是**工作区相对路径**（如 `.clutch/staging/kp.json`）；主机绝对路径（如 `/tmp/...`）会被拒绝并在 Terminal 标明 FORBIDDEN，避免与 agent 写入位置错位时静默失败。
* **Tool Settings**：对 20+ 主流 Agent CLI 白名单做本机探测——**已安装的一律展示**（含 Rivet、OpenCode、MiMo Code、CodeBuddy、Cursor Agent、ZCode、Grok 等扩展工具）；**未安装时默认仅推荐经 Clutch 验证的 CLI**（`codebuddy`、`cursor-agent`、`mimo`、`opencode`、`claude`、`ollama`、`codex`、`agy`、`zcode`）及安装指引（`grok-cli` 为可选旁路引擎，不在默认推荐列表）。CodeBuddy 内置 headless 路由（`codebuddy -p`，curated `--dangerously-skip-permissions`）；OpenCode 内置 headless 路由（`run --auto`）；MiMo Code 内置 headless 路由（`mimo run --dangerously-skip-permissions`）；ZCode 内置 headless 路由（`zcode -p --mode yolo --json`）；Grok 内置 headless 路由（`grok -p`），Auto Config 错误参数不会覆盖 curated 配置。支持 Connect 偏好与 **Auto Config**（LLM 分析 `--help` 写入 `custom_clis.json` 路由参数）。同页 **Command policy**（FM-03）可添加 allow / ask / deny 模式规则：**deny** 直接拒绝（工具结果 `[Permission] Denied`，不弹审批卡）；**ask** 与 `rm -rf` / `sudo` 等危险命令即使 Full 仍 `human_required`。OS 级隔离继续用 General **Strict sandbox**（D21 / OSR-18 界面层）。
* **Model Provider Settings**（**Models by Agent** 顶栏 Tab：**Clutch Agent** · **Claude Code** · **OpenCode** · **MiMo Code** · **More**）：**Clutch** Tab 配置内置 Agent 所用云端/本地模型 API Keys（支持无感导入 `.cc-switch` 凭证至 Clutch 侧）。对话模型在 Chat **底栏随时切换**（Settings 不再分 Planner/Executor，D60）。内置文本提供商含 **DeepSeek**、**Anthropic**、**OpenAI**、**Google**、**Ollama**、**Agnes** 与 **OpenCode Zen**（[opencode.ai](https://opencode.ai/auth) Zen 工作区 API Key；端点 `https://opencode.ai/zen/v1`）。**Claude Code** / **OpenCode** / **MiMo Code** Tab 只读扫描各 CLI 原生 model 配置；Claude Code 在已安装 `cc-switch` CLI 时可切换 provider。**More → Codex**（FM-04）只读扫描 `~/.codex/config.toml` 的当前模型与本地 catalog（**Rescan** 与 Claude/OpenCode 相同；无文件则明确空态）。**Aider / CodeBuddy / Antigravity / Rivet / Ollama / ZCode**（FM-05）同样只读扫描各自本机配置文件（Ollama 读已安装 tag）。**OpenCode Zen**（供 Clutch 内置 Agent，非 OpenCode CLI）仍在 Clutch Tab 配置。内置 **Agnes 3.0 Flash**（对话）、**Agnes Image 2.5 Flash**（生图）与 **Agnes Video 2.5 Flash**（文生视频，720P）；**Ollama 条目与 Create Agent 下拉同源**——实时读取本机 `ollama list` 已安装 tag。
* **Skills Settings**（同上 Agent Tab）：Clutch Tab 管理 Skills Registry 挂载；Claude Code / OpenCode / **MiMo Code** Tab 只读扫描原生 `SKILL.md` 目录。**More → Codex** 同样只读扫描本机 skills 目录（空态可见）。
* **MCP Server Settings**（同上 Agent Tab）：Clutch Tab 注册/开关 MCP Hub 服务器（**stdio 命令或 HTTP URL**，capability **D39 / D57**）。远程 OAuth 与带 Key 的 HTTP 用同一张表。官方 Figma 远程 MCP 只允许目录内客户端（Claude Code / Cursor 等），Clutch 会收到 403、没有授权页；桌面 3845 若已开启仍可 HTTP 直连。见 [`docs/mcp-servers/figma.md`](./mcp-servers/figma.md)。启用非内置服务器时，若 General **未信任确认**开启且该 id 不在本机信任列表，会先弹出确认再启用（FM-02；不做外部 PKI）。每行可 **Test connection**（capability **D38**）——成功显示工具数，失败显示可读错误；不再展示误导性「开发中」横幅。服务器若暴露 MCP **Resources**（capability **D43**）：可 **Browse resources** 并 **Pin for Chat**，快照注入 Clutch Agent 上下文（可 Unpin）。Claude Code / OpenCode / **MiMo Code** Tab 只读扫描原生 MCP 配置。**More → Codex** 只读扫描本机 MCP 配置。
...
* **界面代码层（UI code，D39/D41 / FM-16）**：入口在 **Preview Demo → Coding**。面板 4 步：批准原型（渲染截图 + **Reject** 会 iterate 再改）→ **生成前端工程（确定性 HTML→React，非 AI 重画；每屏 `.tsx` + 同源 Tailwind CDN 主题 + 连线写入路由）** → 预览并批准（小窗缩放 / 完整尺寸打开）→ 交给编码。交付物为会话目录 `react/`：前端 UI + 客户端导航已就绪，后续接 API/联调。若已落盘 `interaction_contract.json`，导航写入源码。不以 `generated/` 交接。Windows 下 preview 会解析 `pnpm.cmd` / `npm.cmd` / `npx.cmd` 完整路径，停止或重新生成时清理 Vite/Node 进程树，避免端口残留。
