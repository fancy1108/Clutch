...
| **分发可信度** | P0 | 无 Apple 签名与公证；无 Release CI；~~版本号仍为 `0.0.0`~~ → **0.1.0**（T0） |
...
| 主题 | 文档表述（当前） | 实现 | 后续 |
|------|------------------|------|------|
| API Key 存储 | 本地 `Application Support/clutch/models.json`，`chmod 600`；UI 密码框输入 | ✅ 一致 | T2：Keychain（OSR-13） |
| Sidecar 仅 localhost | ✅ | ✅ | — |
...
| 项 | 现状 | 建议 |
|----|------|------|
| 版本号 | ~~全局 `0.0.0`~~ → **0.1.0**（T0）；后续单一来源同步 Tauri / Cargo / Python |
...
- [x] OSR-04 版本号 **0.1.0** + `CHANGELOG.md`
