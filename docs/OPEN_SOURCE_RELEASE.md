...
| 类别 | 紧迫度 | 核心问题 |
|------|--------|----------|
| **法律与仓库卫生** | P0 | 无 `LICENSE`；缺少 `SECURITY.md` / `CONTRIBUTING.md`；实验目录含个人路径 |
| **分发可信度** | P0 | 无 Apple 签名与公证；无 Release CI；版本号仍为 `0.0.0` |
...
### 2.2 文档与实现不一致（开源前必须对齐）

| 文档声称 | 实际实现 | 风险 |
|----------|----------|------|
| API Key 存 Tauri secure store（`ARCHITECTURE.md`、`proposal.md` M4-04） | `models.json` 明文 JSON + `chmod 0o600`（`models_config.py`） | 其他本地进程/备份工具可读；与隐私承诺不符 |
| `full` 模式仍拦截 `rm -rf /` 等灾难操作（`preferences_storage.py` 注释） | `mcp_risk.py` 仅按工具名 token 分类，无 shell 命令黑名单 | 误导用户安全预期 |
| FR-07-04 Sidecar 仅 localhost | ✅ 已落实 | — |

开源前须 **二选一**：补齐 secure store 实现，或修订全部产品/架构文档并明确风险告知。

### 2.3 仓库卫生风险

- **`experiments/pty_poc/runs/*.json`**：含 `/Users/fancy/...` 等本机绝对路径与 CLI 探测输出 → 发布前清理或移出公开仓库。
- **`.claude/settings.local.json`**：含个人路径；应加入 `.gitignore`，并提供 `.claude/settings.local.json.example`。
- **无 `LICENSE`**：他人无法合法 fork、打包、再分发。
- **版本 `0.0.0`**：`tauri.conf.json`、`Cargo.toml`、`pyproject.toml` 均未 semver 化 → 用户无法判断兼容性，也无法做安全公告关联。
...
| 项 | 现状 | 建议 |
|----|------|------|
| 版本号 | 全局 `0.0.0` | 单一来源（如 `package.json`）同步到 Tauri / Cargo / Python |
...
| OSR-04 | semver 版本号 + `CHANGELOG.md` 起步 | **T0** | — | 0.5d | 从 `0.1.0` 起；与功能迭代同步打 tag |
...
| OSR-11 | Apple 签名 + 公证（notarytool） | **T2** | 1 | 2–3d | 需 Apple Developer 账号 |
| OSR-12 | GitHub Actions `release.yml`（macOS runner 打 DMG） | **T2** | 1 | 2–3d | 依赖 OSR-11 |
...
| 做 | 不做（暂排） |
|----|----------------|
| OSR-01～05、**OSR-23～25**（T0 护栏 + 生态边界文档） | OSR-11～12 签名与 Release CI |
...
- [ ] OSR-02 仓库卫生
- [ ] OSR-03 文档↔实现一致
- [ ] OSR-04 版本号 + `CHANGELOG.md`
- [ ] OSR-05 新代码安全护栏（review 项）
- [ ] OSR-23 `PROJECT_SCOPE.md`
- [ ] OSR-24 `STABILITY.md`
- [ ] OSR-25 `EXTENSIBILITY.md`
...
- [ ] OSR-09 CLI 权限策略
- [ ] OSR-10 README / 构建文档
- [ ] OSR-26 `GOVERNANCE.md`
- [ ] OSR-27 兼容性矩阵 + `doctor.sh`
- [ ] OSR-28 Issue / PR 模板
