...
| 步骤 | 谁做 | 说明 |
|------|------|------|
| 1. `CHANGELOG.md` 增加 `## [x.y.z]` | AI | 开发中写入当前目标节（现为 `## [Unreleased]`）；打 tag 时改日期、bump `package.json` / Tauri，并开下一空目标节。`release-preflight.sh` 会检查（INV-R5）；该节会成为 **GitHub Release 正文顶部**（见 §Release 正文结构） |
