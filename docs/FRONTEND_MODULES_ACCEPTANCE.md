...
**入口：** 底栏 **Branch** 右边的 **Worktree**（`footer-worktree-trigger`）。也可 **+** → **Enable worktree**。

工作区必须是 git（底栏有 Branch）。**改完 Sidecar 后先完全退出再开 App**。

| # | 操作 | 期望 |
|---|------|------|
| 1 | 未 Enable | 底栏 `Worktree: —`；打开菜单没有 Add parallel；输入栏上方没有天蓝条 |
| 2 | + 或底栏菜单 → Enable worktree | Supervisor：「Worktree isolation enabled at …/.clutch/worktrees/wt_…」；底栏变成 `Worktree: clutch/wt_…`（`worktree-active-chip`） |
| 3 | 底栏打开菜单，点该隔离行右侧 **Discard** 图标 | Supervisor：「Discarded worktree …」；底栏回到 `—`；`.clutch/worktrees/wt_…` 目录没了 |
| 4 | 再 Enable，让 Agent 在隔离目录写文件 | 主仓 `git status` 仍干净；菜单该项可带 dirty |
| 5 | 启用后打开底栏菜单 **Add parallel worktree**（在切换列表下方） | 按钮立刻转圈「正在创建…」，菜单不关；成功后多一条 `clutch/wt_…`，可点选切换；Merge/Discard 在该行右侧 |
| 5b | 切到某一棵隔离树 | 右侧 **Files** / **Changes** 变成那棵树的内容；切回 **Main workspace** 回到主仓。底栏 **Branch** 仍是主仓分支 |
| 5c | 在隔离树改 README（不必先手动 commit）再点该行 **Merge** | Supervisor 说已合进主仓；**树还在**；切回 Main 后 Files 里 README 有那处改动。要删树才点 Discard |
| 6 | 非 git 工作区 Enable | 可读错误（工作区不是 git），不是空白 |

**不过：** 改 Sidecar 后必须完全退出再开 App（无 `--reload`）。只建文件时 Agent 不得 `git commit`（含终端）。Files/Changes 跟着底栏所选 Worktree：隔离树里的新文件只在切到该树时出现，切回 **Main workspace** 看不到；底栏 **Branch** 仍是主仓。

**复制：** 底栏有 Branch 时，Chat **+** → Enable worktree（或底栏 Worktree 菜单 Enable）。Discard 测完再 Enable，然后发送：
