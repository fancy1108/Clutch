...
# E2E Tauri dev: start Vite (like tauri-dev), inject playwright capability, then tauri.
set -euo pipefail
...
VITE_LOG="${REPO_ROOT}/.clutch-vite-e2e.log"
VITE_PID_FILE="${REPO_ROOT}/.clutch-vite-e2e.pid"
...
if ! curl -sf "$VITE_URL" >/dev/null 2>&1; then
  ...
  for _ in $(seq 1 120); do
    ...
    if [[ -f "$VITE_PID_FILE" ]]; then
      ...
      if ! kill -0 "$VITE_PID" 2>/dev/null; then
        echo "[tauri-e2e] Vite exited before :3000 became ready. Log:" >&2
  ...
  if ! curl -sf "$VITE_URL" >/dev/null 2>&1; then
    echo "[tauri-e2e] Timed out waiting for Vite on :3000. Log:" >&2
    tail -30 "$VITE_LOG" >&2 || true
    exit 1
  fi
  echo "[tauri-e2e] Vite ready at $VITE_URL (pid $(cat "$VITE_PID_FILE"), log $VITE_LOG)"
else
  echo "[tauri-e2e] Reusing existing Vite on :3000"
fi

cp "$E2E_CAP" "$E2E_LINK"
export CLUTCH_RUNTIME_MODE="${CLUTCH_RUNTIME_MODE:-hybrid}"
cd "$DESKTOP"
exec pnpm tauri dev --features e2e-testing --config src-tauri/tauri.e2e.conf.json --no-dev-server-wait "$@"
