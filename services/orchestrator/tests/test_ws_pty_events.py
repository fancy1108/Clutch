...
def test_ws_pty_attach_blocked_without_workspace() -> None:
    with patch("src.workspace.get_workspace", return_value=None):
        with patch("src.workspace.active_workspace_issue", return_value="No workspace authorized for interactive PTY"):
...
def test_ws_pty_attach_blocked_when_workspace_path_missing() -> None:
    stale = "/tmp/clutch-missing-workspace-path"
    with patch(
        "src.workspace.get_workspace",
        return_value=None,
    ):
        with patch(
            "src.workspace.active_workspace_issue",
            return_value=f"Workspace folder no longer exists: {stale}",
        ):
            with client.websocket_connect("/ws/runs/run_pty_stale_ws") as ws:
                ws.receive_json()
                ws.send_json({"action": "pty_attach", "cli_tool": "claude-cli", "lane_id": "lane_primary"})
                status = _receive_pty_status(ws)
    assert status["data"]["status"] == "blocked"
    assert stale in status["data"]["message"]
...
def test_ws_pty_attach_and_detach(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch("src.workspace.get_workspace", return_value={"workspace_path": "/tmp"}):
        with patch("src.workspace.active_workspace_issue", return_value=None):
            with patch(
