...
def test_manager_busy_rejects_while_creating(monkeypatch: pytest.MonkeyPatch) -> None:
    manager = ShellSessionManager()
    session = ShellSession(
        run_id="run-a",
        workspace_path="/tmp",
...
def test_parallel_get_or_create_different_runs(monkeypatch: pytest.MonkeyPatch) -> None:
    """PTY spawn must not hold the manager lock (concurrent plain-chat sessions)."""
    import threading
    ...
    def slow_spawn(self: ShellSession) -> None:
    ...
    monkeypatch.setattr(ShellSession, "_spawn", slow_spawn)

    results: list[ShellSession] = []
    errors: list[BaseException] = []

    def acquire(run_id: str) -> None:
        try:
            results.append(manager.get_or_create(run_id, workspace_path="/tmp"))
