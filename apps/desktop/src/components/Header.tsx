...
interface HeaderProps {
  ...
  onGoBack?: () => void;
  setView: (view: 'chat' | 'workflows' | 'settings' | 'agents') => void;
  sidebarOpen?: boolean;
}
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const parentFolder = folders?.find(folder => 
    folder.items.some((item: any) => 
      ...
      (item.name === 'Clutch workspace...' && currentFlow === 'Video Production')
    )
  );
  ...
  return (
    <header className={`fixed top-0 right-0 h-[64px] bg-white/85 backdrop-blur-md border-b border-outline-variant z-40 flex items-center justify-between px-10 select-none transition-all duration-300 ${
